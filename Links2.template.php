...
function template_mainview()
{
	...
	if (empty($cat))
	{
		...
		echo '
        <div class="cat_bar">
		<h3 class="catbg centertext">Smf Dizinleri</h3>
  </div>
  <br />
		...
		foreach($context['links_cats'] as $row)
		{
			...
			echo '<div class="smfdizin">';

			if ($row['image'] == '')
				echo '<p><a href="' . $scripturl . '?action=links;cat=' . $row['ID_CAT'] . '">' . parse_bbc($row['title']) . '</a></p><p>' . parse_bbc($row['description']) . '</p>';
			else
			{
				echo '<p><a href="' . $scripturl . '?action=links;cat=' . $row['ID_CAT'] . '"><img src="' . $row['image'] . '" border="0" alt="" /></a></p>';
				echo '<p><a href="' . $scripturl . '?action=links;cat=' . $row['ID_CAT'] . '">' . parse_bbc($row['title']) . '</a></p><p>' . parse_bbc($row['description']) . '</p>';
			}


			echo '' . $txt['smflinks_totallinks'] . ':', $totallinks, '';

			// Show Edit Delete and Order category
			if ($m_cats)
				echo '<p><a href="' . $scripturl . '?action=links;sa=catup;cat=' . $row['ID_CAT'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '">' . $txt['smflinks_txtup'] . '</a>&nbsp;<a href="' . $scripturl . '?action=links;sa=catdown;cat=' . $row['ID_CAT'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '">' . $txt['smflinks_txtdown'] . '</a></span>&nbsp;<a href="' . $scripturl . '?action=links;sa=catperm;cat=' . $row['ID_CAT'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '">' . $txt['smflinks_txt_perm'] . '</a>&nbsp;<a href="' . $scripturl . '?action=links;sa=editcat;cat=' . $row['ID_CAT'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '">' . $txt['smflinks_txtedit'] . '</a>&nbsp;<a href="' . $scripturl . '?action=links;sa=deletecat;cat=' . $row['ID_CAT'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '">' . $txt['smflinks_txtdel'] . '</a></p>';
			...
			if ($subcats_linktree != '')
				echo '<div class="titlebg">
					<p>&nbsp;<span class="smalltext">',($subcats_linktree != '' ? $txt['smflinks_sub_cats'] . $subcats_linktree : ''),'</span></p>
				</div>';
  ...
  <tr>
    <td width="50%" valign="top">';
		...
		if (!empty($modSettings['smflinks_setshowtoprate']))
		{
			echo '
            <div class="cat_bar">
		<h3 class="catbg centertext">En İyi Beş Bağlantı</h3>
  </div>
            ';
	...
	echo '</td>';
		// Show Top 5 hits
	echo '<td width="49%" style="padding-left:15px;" valign="top">';
		if (!empty($modSettings['smflinks_setshowmostvisited']))
		{

			echo '<div class="cat_bar">
		<h3 class="catbg centertext">En çok Ziyaret Edilenler</h3>
  </div>';
